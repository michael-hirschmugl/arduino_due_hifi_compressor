...
/*
  ...
  returns 0 if OK
*/
uint8_t SAM3X8E_TWIClass::send_two_bytes(uint8_t dadr, uint8_t byte1, uint8_t byte2)
{
    uint16_t i = 0;
    TWI1_THR = byte1;
    ...
    /*
        ...
        The WM8731 has a hardwired adress of 0x1A.
    */
    TWI1_MMR = TWI1_MMR_MREAD | (dadr << 16);
...
//    uint16_t i = 0;
//    
//    i |= (send_two_bytes(0x1AU, 0x0CU, ((PWROFF << 7)|(CLKOUTPUT << 6)|(OSCPD << 5)|(OUTPD << 4)|(DACPD << 3)|(ADCPD << 2)|(MICPD << 1)|(LINEINPD << 0))) << 0 );
//    i |= (send_two_bytes(0x1AU, 0x0EU, ((BCLKINV << 7)|(MS << 6)|(LRSWAP << 5)|(LRP << 4)|(IWL << 2)|(FORMAT << 0))) << 1);
//    i |= (send_two_bytes(0x1AU, 0x00U, ((LRINBOTH << 8)|(LINMUTE << 7)|(LINVOL << 0))) << 2);
//    i |= (send_two_bytes(0x1AU, 0x02U, ((RLINBOTH << 8)|(RINMUTE << 7)|(RINVOL << 0))) << 3);
//    i |= (send_two_bytes(0x1AU, 0x04U, ((LRHPBOTH << 8)|(LZCEN << 7)|(LHPVOL << 0))) << 4);
//    i |= (send_two_bytes(0x1AU, 0x06U, ((RLHPBOTH << 8)|(RZCEN << 7)|(RHPVOL << 0))) << 5);
//    i |= (send_two_bytes(0x1AU, 0x0AU, ((HPOR << 4)|(DACMU << 3)|(DEEMPH << 1)|(ADCHPD << 0))) << 6);
//    i |= (send_two_bytes(0x1AU, 0x08U, ((SIDEATT << 6)|(SIDETONE << 5)|(DACSEL << 4)|(BYPASS << 3)|(INSEL << 2)|(MUTEMIC << 1)|(MICBOOST << 0))) << 7);
//    i |= (send_two_bytes(0x1AU, 0x10U, ((CLKODIV2 << 7)|(CLKIDIV2 << 6)|(SR << 2)|(BOSR << 1)|(USB << 0))) << 8);
//    i |= (send_two_bytes(0x1AU, 0x12U, (ACTIVE << 0)) << 9);
//
//    return i;
//}
